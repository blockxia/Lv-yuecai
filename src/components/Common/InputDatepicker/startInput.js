...
export default class StartDatePicker extends Component {
  ...
  */
  componentDidMount() {
    ...
    // 日历选择后回调
    $('.date-picker-start').datepicker().off('changeDate').on('changeDate', function(e){
      let selectDate = e.date;
      that.props.changeDate && that.props.changeDate(e);
      $('.date-picker-start').datepicker('update');
    });
  }

  render() {
    return (
      <Input type="text" readOnly id="date" className="date-picker-start form-control input-sm input-icon-calendar w120 js-startDate" data-date={this.props.currentDate} value={this.props.currentDate} data-currentOrder={this.props.currentOrder} />
    );
  }
}
