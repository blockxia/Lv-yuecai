...
class ModifyDistributionRole extends Component {
    ...
    if(isEmptyObject(checkMenuItem.childMap)){
      return (
        <li className="checkbox-list" key={checkMenuItem.node.id}>
          <div className="arrow"></div><div className={checkMenuItem.node.checked?'checkbox checked':'checkbox'}></div>
      ...
      if(isEmptyObject(childMenuItem[key].childMap)){
         tamp.push(
          <li className="checkbox-wrap-list clearfix" key={childMenuItem[key].node.id}>
             <div className="arrow"></div><div className={childMenuItem[key].node.checked?'checkbox checked':'checkbox'}></div>
   ...
   for(var key in item){
     options.push(
         <li key={item[key].node.id}>
            <div className="arrow"></div><div className={item[key].node.checked?'checkbox checked':'checkbox'}></div>
 ...
 savePowerRole(e){
  var checkedArray = [];
  var _this = this;
  var res1 = $('ul.checktree_two').find('.checked').next("input");
  var res2= $('ul.checktree_two').find('.half_checked').next("input");
 ...
 componentDidMount(){
    if(this.props.visible && $("ul.checktree_two").length){
      $("ul.checktree_two").checkTree();
      $("ul.checktree_two").find('input[data-checked=true]').prev().addClass('checked');
    }
 }
    ...
    return (
      <div>
      {
        this.props.visible?
          ...
          >
            <div className="modify-distribution-container">
              <div className="distribution-warpper">
                ...
                <div className="checktree-wrapper">
                  ...
                  <form>
                    <ul className="checktree_two">
                      {this.getDom()}
                    </ul>
                  </form>
                </div>
                </div>
                <p className="save_bt_sublimt">
                  <Button type="primary" loading={this.state.loading} onClick={this.savePowerRole.bind(this)}>{intl.get('lv.common.saveSettings')}</Button>
                </p>
            </div>
          </Modal>
          :''}
...
export default connect(mapStateToProps, mapDispatchToProps,)(ModifyDistributionRole);
